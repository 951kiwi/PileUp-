...
public class RsStreamRenderer : MonoBehaviour
{
    ...
    // 深度データの生データをbyte[]で渡すイベント
    public event Action<byte[]> OnDepthDataReceived;
    private byte[] depthDataBuffer;
    ...
    private void ProcessFrame(VideoFrame frame)
    {
        ...
        if (frame.Profile.Stream == Stream.Depth && frame.Profile.Format == Format.Z16)
        {
            int width = frame.Width;
            int height = frame.Height;
            int length = width * height;

            ushort[] depthData = new ushort[length];
            IntPtr dataPtr = frame.Data;

            // メモリコピー
            System.Runtime.InteropServices.Marshal.Copy(dataPtr, (short[])(object)depthData, 0, length);

            // GameManager に送信
            if (GameManager.Instance != null)
            {
                GameManager.Instance.HandleDepthFrame(depthData);
            }
        }
    }
}
